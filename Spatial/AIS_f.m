function [cover_noise] = AIS_f(cover,gamma,delta,NP,G,Nc1,e,Isr,QualityFactor)
NR=12;
...
len=zeros(1,NP);                                 
for i=1:NP 
    len(:,i) = fitness_func(f(:,i), cover,QualityFactor);
end
...
while gen < G
    for i=1:floor(NP*Isr)        
        a=Sortf(:,i);         
        Ca=repmat(a,1,Nc1);                
        for j = 1:Nc1
            sigma = textureResidual.*e;
        ...
        Ca(:,1)=Sortf(:,i);                
        for j=1:Nc1
           Calen(j,:) = fitness_func(Ca(:,j), cover,QualityFactor);       
        end
    ...
    for i = 1:NP-floor(NP/4)
        ...
        blen(i) = fitness_func(bf(:,i), cover, QualityFactor);
    end
    f=[af,bf]; 
    len=[alen,blen];
