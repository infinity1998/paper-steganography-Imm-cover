function x = SUNIWARD_STC(coverPath,payload)
cover = imread(coverPath);
% n = size(cover,1)* size(cover,2);
n = 256*256;
m = floor(n/(1/payload));  % embed m bits
h = 10;   % use STCs with constraint_height = 10. Try values between 7 and 12.

cover = int32(reshape(cover, 1, size(cover,1)*size(cover,2)));
msg = uint8(rand(1,m));        % generate m random message bits

[rhoP1,rhoM1,rho0] = S_UNIWARD_STC(cover);
rho0 = single(zeros(size(rhoM1,1),size(rhoM1,2)));
costs = [rhoM1;rho0;rhoP1];
