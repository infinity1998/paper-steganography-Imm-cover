...
tic
dirSource = 'Uimmcover_qf75';             % input dir for all QFs
output_dir = 'stego_stc';                       % output dir of all schemes 
  files=dir([dirSource '\*.jpg']);
    for w=1:length(files)        
            ...
            xx = int32(nz_dct_coef)';
            [d stego n_msg_bits l] = stc_pm1_pls_embed(xx, costs, uint8(hidden_message)', H); % ternary STC embedding 
            extr_msg = stc_ml_extract(stego, n_msg_bits, H);

            ttt = sum(uint8(hidden_message)'~=extr_msg);
