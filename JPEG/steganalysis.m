...
% quality factor = 75.
clear all;
load('Steganalysis_Feature\cover.mat');
load('Steganalysis_Feature\stego.mat');
...
fprintf("OOB:%d L:%d\n",results.optimal_d_sub,results.optimal_L);
DSUB = results.optimal_d_sub;
OL = results.optimal_L;
...
settings = struct('d_sub',DSUB,'L',OL,'verbose',2);
min_test_error = 0.5;
for seed = 1:10
    ...
    FL(seed) = (false_alarms)/size(TST_cover,1);
    MD(seed) = (missed_detections)/size(TST_stego,1);
    
    if testing_errors(seed) < min_test_error
        min_test_error = testing_errors(seed);
        fprintf('最优正确率为：%.4f\n',min_test_error);
    end
...
fprintf('false_alarms:%.4f (+/- %.4f)\n',mean(FL),std(FL));
fprintf('missed_detections:%.4f (+/- %.4f)\n',mean(MD),std(MD));
